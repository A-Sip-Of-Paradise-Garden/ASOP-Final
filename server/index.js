...
app.post('/create-donate-checkout-session', cors(), async (req, res) => {
    const session = await stripe.checkout.sessions.create({
        ...
        mode: 'payment',
        success_url: `http:/45.79.199.14:3000/donation-payment-success`, // Change based on hosted url
        cancel_url: `http://45.79.199.14:3000/`,  // Change based on hosted url
    });
...
app.post('/create-dues-checkout-session', cors(),async (req, res) => {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        mode: 'payment',
        success_url: `http:/45.79.199.14:3000/dues-payment-success`, // Change based on hosted url
        cancel_url: `http://45.79.199.14:3000/`,  // Change based on hosted url
        metadata: {userId}
    });
